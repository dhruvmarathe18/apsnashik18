...
export function exportBusReportToExcel(reportData: any) {
...
export function exportDriverReportToExcel(reportData: any) {
...
export function exportAllDataToExcel(transportData: any, BUS_NAMES: string[]) {
  const wb = XLSX.utils.book_new()
  ...
  BUS_NAMES.forEach(bus => {
    ...
    if (data.length > 0) {
      ...
      const sheetData = [headers, ...data.map((entry: any) => [
        entry.Date,
        entry['Driver Name'] || '',
        entry['Start KM'] || 0,
        entry['End KM'] || 0,
